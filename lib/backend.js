...
export async function createAccount(email, nickname, fname, lname, neighbourhood) {
	...
	const createStatement = await db.prepare(
		"INSERT INTO users (email, nickname, fname, lname, neighbourhood, rank, position) VALUES (?, ?, ?, ?, ?, ?, ?)"
	);

	await createStatement.run(email, nickname, fname, lname, neighbourhood, rank, position);
...
export async function createInviteCodeFor(neighbourhood) {
	const db = await database;
	const createStatement = await db.prepare(
		"INSERT INTO invite_codes (code, neighbourhood) VALUES (?, ?)"
	);

	const code = randomBytes(4).toString("base64url").substring(0, 5);
	console.log(`GENERATED INVITE CODE: ${code}`);

	await createStatement.run(code, neighbourhood);
	await createStatement.finalize();
	return code;
}
...
export async function getEmailsForTarget(notif, neighbourhood) {
	...
	if(notif.target == NotificationTarget.User) {
		const getUserStatement = await db.prepare(
			"SELECT email FROM users WHERE uid=? AND email_notifications=1"
		);

		const user = await getUserStatement.get(notif.userId);
		await getUserStatement.finalize();

		if(!user) throw new Error("User does not exist or has email notifications turned off");

		return [ user.email ];
	} else if(neighbourhood) {
		const getUsersStatement = await db.prepare(
			"SELECT email FROM users WHERE neighbourhood=? AND position IN (?) AND email_notifications=1"
		);
		...
		const getUsersStatement = await db.prepare(
			"SELECT email FROM users WHERE position IN (?) AND email_notifications=1"
		);
...
export async function getPostData(pid) {
	...
	const getPostStatement = await db.prepare(
		"SELECT * FROM (SELECT * FROM posts INNER JOIN users ON author=uid) WHERE pid=?"
	);
...
export async function updateUserData(uid, nickname, fname, lname, bio, email_notifications) {	
	const db = await database;
	const updateUserDataStatement = await db.prepare(
		"UPDATE users SET nickname=?, fname=?, lname=?, bio=?, email_notifications=? WHERE uid=?"
	);

	await updateUserDataStatement.run(nickname, fname, lname, bio, email_notifications, uid);
