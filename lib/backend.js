...
export async function createAccount(email, nickname, dob, fname, lname) {
	...
	try {
		...
		await statement.run(email, nickname, dob, fname ?? null, lname ?? null, Date.now());
		statement.finalize();
...
export function getNotificationsFor(user) {
	try {
		...
		const getNotificationsStatement = await db.prepare(
			"SELECT uid, content FROM notifications WHERE ..."	
		);
	} catch(e) {
		console.error(e);
		return [];
...
export async function getUserData(user) {
	try {
		...
		const userData = await getUserData.get(uid);
...
export async function updateProfile(user, nickname, fname, lname, bio) {
	try {
		...
		await updateStatement.run(nickname, fname, lname, bio, uid);
		return true;
	} catch(e) {
		console.error(e);
		return false;
